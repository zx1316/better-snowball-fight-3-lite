...
import com.linngdu664.bsf3lite.entity.snowball.nomal.ObsidianSnowballEntity;
...
public class ObsidianSnowballItem extends AbstractBSFSnowballItem implements ProjectileItem {
    public ObsidianSnowballItem() {
        super("obsidian_snowball", Rarity.COMMON, new SnowballProperties()
                .idForTank(7)
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, @NotNull Position position, @NotNull ItemStack itemStack, @NotNull Direction direction) {
        ObsidianSnowballEntity snowball = new ObsidianSnowballEntity(level, position.x(), position.y(), position.z(), itemStack.get(DataComponentRegistry.REGION));
    ...
    public void addMainTips(Consumer<Component> builder) {
        builder.accept(Component.translatable("obsidian_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
