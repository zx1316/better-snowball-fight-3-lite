...
import com.linngdu664.bsf3lite.entity.snowball.nomal.StoneSnowballEntity;
...
public class StoneSnowballItem extends AbstractBSFSnowballItem implements ProjectileItem {
    public StoneSnowballItem() {
        super("stone_snowball", Rarity.COMMON, new SnowballProperties()
                .idForTank(2)
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, @NotNull Position position, @NotNull ItemStack itemStack, @NotNull Direction direction) {
        StoneSnowballEntity snowball = new StoneSnowballEntity(level, position.x(), position.y(), position.z(), itemStack.get(DataComponentRegistry.REGION));
    ...
    public void addMainTips(Consumer<Component> builder) {
        builder.accept(Component.translatable("stone_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
