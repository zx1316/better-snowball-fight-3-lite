...
import com.linngdu664.bsf3lite.entity.snowball.special.SpectralSnowballEntity;
...
public class SpectralSnowballItem extends AbstractBSFSnowballItem implements ProjectileItem {
    public SpectralSnowballItem() {
        super("spectral_snowball", Rarity.UNCOMMON, new SnowballProperties().idForTank(9).allowLaunchTypeFlag(AbstractBSFSnowballItem.HAND_TYPE_FLAG | SnowballCannonItem.TYPE_FLAG | SnowballShotgunItem.TYPE_FLAG | SnowballMachineGunItem.TYPE_FLAG));
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, @NotNull Position position, @NotNull ItemStack itemStack, @NotNull Direction direction) {
        SpectralSnowballEntity snowball = new SpectralSnowballEntity(level, position.x(), position.y(), position.z(), itemStack.get(DataComponentRegistry.REGION));
    ...
    public void addMainTips(Consumer<Component> builder) {
        builder.accept(Component.translatable("spectral_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
