...
import com.linngdu664.bsf3lite.entity.snowball.special.CriticalFrozenSnowballEntity;
...
public class CriticalFrozenSnowballItem extends AbstractBSFSnowballItem implements ProjectileItem {
    public CriticalFrozenSnowballItem() {
        super("critical_frozen_snowball", Rarity.RARE, new SnowballProperties()
                .idForTank(12)
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, @NotNull Position position, @NotNull ItemStack itemStack, @NotNull Direction direction) {
        CriticalFrozenSnowballEntity snowball = new CriticalFrozenSnowballEntity(level, position.x(), position.y(), position.z(), itemStack.get(DataComponentRegistry.REGION));
    ...
    public void addMainTips(Consumer<Component> builder) {
        builder.accept(Component.translatable("critical_frozen_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
