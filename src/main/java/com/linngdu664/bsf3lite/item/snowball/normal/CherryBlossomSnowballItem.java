...
import com.linngdu664.bsf3lite.entity.snowball.nomal.CherryBlossomSnowballEntity;
...
public class CherryBlossomSnowballItem extends AbstractBSFSnowballItem implements ProjectileItem {
    public CherryBlossomSnowballItem() {
        super("cherry_blossom_snowball", Rarity.COMMON, new SnowballProperties().idForTank(1).allowLaunchTypeFlag(AbstractBSFSnowballItem.HAND_TYPE_FLAG | SnowballCannonItem.TYPE_FLAG | SnowballShotgunItem.TYPE_FLAG | SnowballMachineGunItem.TYPE_FLAG));
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, @NotNull Position position, @NotNull ItemStack itemStack, @NotNull Direction direction) {
        CherryBlossomSnowballEntity snowball = new CherryBlossomSnowballEntity(level, position.x(), position.y(), position.z(), itemStack.get(DataComponentRegistry.REGION));
    ...
    public void addMainTips(Consumer<Component> builder) {
        builder.accept(Component.translatable("cherry_blossom_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
